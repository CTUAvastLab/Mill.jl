...
"""
struct MaybeHotVector{T, U} <: AbstractVector{U}
    i::T
    l::Int

    MaybeHotVector(i::T, l::Int) where T <: Integer = new{T, Bool}(i, l)
    MaybeHotVector(i::T, l::Int) where T <: Missing = new{T, Missing}(i, l)
    MaybeHotVector(i::T, l::Int) where T <: Maybe{Integer} = new{T, Maybe{T}}(i, l)

    function MaybeHotVector{T, U}(i, l) where {T <: Maybe{Integer}, U <: Maybe{Bool}}
        new{T, U}(convert(T, i), l)
    end
end
...
function Base.convert(::Type{MaybeHotVector{T, U}}, x::MaybeHotVector) where {T, U}
    MaybeHotVector{T, U}(convert(T, x.i), x.l)
end

function Base.promote_rule(::Type{MaybeHotVector{T, U}},
        ::Type{MaybeHotVector{A, B}}) where {T, A, U, B}
    MaybeHotVector{promote_type(T, A), promote_type(U, B)}
end
...
_mul(A::AbstractMatrix, ::MaybeHotVector{Missing}) = fill(missing, size(A, 1))
...
maybecold(::MaybeHotVector{Missing}, labels=nothing) = missing
