...
function mgradcheck(f, xs...)
    ...
    for (ng, ag) in zip(mngradient(f, xs...), gradient(f, xs...))
        ...
        elseif !all(isapprox.(collect(ng), collect(ag), rtol = 1e-5, atol = 1e-5))
            correct = false
...
function gpu_gradtest(f, xs...)
    f_gpu = f |> Mill.mgpu
    xs_gpu = xs .|> Mill.mgpu

    cpugrad = gradient((xs...) -> sum(sin.(f(xs...))), xs...)
    gpugrad = gradient((xs_gpu...) -> sum(sin.(f_gpu(xs_gpu...))), xs_gpu...)

    correct = true
    for (cg, gg) in zip(cpugrad, gpugrad)
        if !isnothing(gg) ; gg = collect(gg) ; end
        if isnothing(gg)
            if !isnothing(cg) && any(cg .!= 0)
                correct = false
                @show cg
            end
        elseif !all(isapprox.(cg, gg, rtol = 1e-5, atol = 1e-5))
            correct = false
            grad_dif = [abs.(x) for x in (cg .- gg)]
            @show cg
            @show gg
            @show grad_dif
        end
...
@testset "aggregation grad check w.r.t. input" begin
    for bags in BAGS
        ...
        for idxs in powerset(collect(1:length(anames)))
            ...
            @test gpu_gradtest(a, x, bags)
            @test gpu_gradtest(a, x, bags, w)
            @test gpu_gradtest(a, x, bags, w_mat)
        end
    end
end
