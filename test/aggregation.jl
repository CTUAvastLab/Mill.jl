...
@testset "aggregation functionality" begin
	...
	@test segmented_meanmax(X, BAGS) ≈ cat(1, segmented_mean(X, BAGS), segmented_max(X, BAGS))
	@test segmented_mean(X, BAGS, W) ≈ [1.0 4.0 236/24; 2.0 5.0 260/24]
	@test segmented_max(X, BAGS, W) ≈ segmented_max(X, BAGS)
	@test segmented_meanmax(X, BAGS, W) ≈ cat(1, segmented_mean(X, BAGS, W), segmented_max(X, BAGS, W))
