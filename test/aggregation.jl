...
import Mill: AggregationModel, ChainModel
...
@testset "aggregation functionality" begin
	...
	@test segmented_pnorm(x2, bags2, [1.0, 1.0], [0.0, 0.0]) ≈ segmented_mean(abs.(x2), bags2)
	@test segmented_pnorm(x2, bags2, [2.0, 2.0], [0.0, 0.0]) ≈ hcat([sqrt.(sum(x2[:, b] .^ 2, 2) ./ length(b)) for b in bags2]...)
end
