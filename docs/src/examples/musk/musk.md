...
y = map(i -> maximum(y[i]) + 1, bagids)   # create labels on bags
y = Flux.onehotbatch(y, 1:2)              # one-hot encoding
```
...
```@repl musk
loss(ds, y) = Flux.logitcrossentropy(model(ds).data, y)
```

and run simple training procedure using its tooling:

```@repl musk
evalcb = () -> @show(loss(ds, y))
opt = Flux.ADAM()
@epochs 10 Flux.train!(loss, params(model), repeated((ds, y), 1000), opt, cb=throttle(evalcb, 1))
```
