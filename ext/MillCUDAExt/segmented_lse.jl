...
function kernel_lse_precomp_aligned!(M, x, r, bags)
    idx = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    n_rows, n_cols = size(M)
...
function kernel_segmented_lse_forw_aligned!(o, x, ψ, r, M, bags)
    idx = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    n_rows, n_cols = size(o)
...
function kernel_segmented_lse_back_aligned!(dx, dr, dψ, Δ, y, x, r, M, bags)
    idx = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    n_rows, n_cols = size(Δ)
...
function lse_precomp_gpu(x::CuMatrix, r::CuVector, bags::CuAlignedBags)
    M = CUDA.zeros(eltype(x), size(x, 1), length(bags.bags))
    n_elements = length(M)
    if n_elements > 0
        threads, blocks = compute_grid_config(n_elements)
        @cuda threads=threads blocks=blocks kernel_lse_precomp_aligned!(M, x, r, bags.bags)
    end
    M
end

function segmented_lse_forw_with_M(x::CuMatrix, ψ::CuVector, r::CuVector, bags::CuAlignedBags)
    M = lse_precomp_gpu(x, r, bags)
    o = CUDA.zeros(eltype(x), size(x, 1), length(bags.bags))
    n_elements = length(o)
    if n_elements > 0
        threads, blocks = compute_grid_config(n_elements)
        @cuda threads=threads blocks=blocks kernel_segmented_lse_forw_aligned!(o, x, ψ, r, M, bags.bags)
    end
    o, M
end

function segmented_lse_forw(x::CuMatrix, ψ::CuVector, r::CuVector, bags::CuAlignedBags)
    segmented_lse_forw_with_M(x, ψ, r, bags)[1]
end

segmented_lse_forw(::Missing, ψ::CuVector, r::CuVector, bags::CuAlignedBags) = repeat(ψ, 1, length(bags.bags))

function segmented_lse_back(Δ::CuMatrix, y::CuMatrix, x::CuMatrix, ψ::CuVector, r::CuVector,
                            bags::CuAlignedBags, M::CuMatrix)
    dx = CUDA.zeros(eltype(x), size(x))
    dr = CUDA.zeros(eltype(r), length(r))
    dψ = CUDA.zeros(eltype(ψ), length(ψ))
    ...
    if n_elements > 0
        threads, blocks = compute_grid_config(n_elements)
        @cuda threads=threads blocks=blocks kernel_segmented_lse_back_aligned!(dx, dr, dψ, Δ, y, x, r, M, bags.bags)
    end

    dx, dψ, dr, ChainRulesCore.NoTangent()
end

function segmented_lse_back(Δ::CuMatrix, x::Missing, ψ::CuVector, bags::CuAlignedBags)
    dψ = CUDA.zeros(eltype(ψ), length(ψ))
    n_elements = length(Δ)
    if n_elements > 0
        threads, blocks = compute_grid_config(n_elements)
...
function ChainRulesCore.rrule(::typeof(segmented_lse_forw), x::CuMatrix, ψ::CuVector, r::CuVector, bags::CuAlignedBags)
    y, M = segmented_lse_forw_with_M(x, ψ, r, bags)
    function segmented_lse_pullback(Δ)
        Δ_unthunked = ChainRulesCore.unthunk(Δ)
        grads = segmented_lse_back(Δ_unthunked, y, x, ψ, r, bags, M)
...
function ChainRulesCore.rrule(::typeof(segmented_lse_forw), x::Missing, ψ::CuVector, r::CuVector, bags::CuAlignedBags)
    y = segmented_lse_forw(x, ψ, r, bags)
    function segmented_lse_pullback(Δ)
        Δ_unthunked = ChainRulesCore.unthunk(Δ)
        grads = segmented_lse_back(Δ_unthunked, x, ψ, bags)
