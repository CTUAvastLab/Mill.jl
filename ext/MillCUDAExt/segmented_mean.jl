...
function kernel_segmented_mean_forw_aligned!(o, x, ψ, bags)
    idx = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    n_rows, n_cols = size(o)
...
function kernel_segmented_mean_forw_w_aligned!(o, x, ψ, w, bags)
    idx = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    n_rows, n_cols = size(o)
...
function kernel_segmented_mean_back_aligned!(dx, Δ, bags)
    idx = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    n_rows, n_cols = size(Δ)
    ...
    if idx <= n_rows * n_cols
        ...
        if !isempty(bag)
            bag_len = length(bag)
            @inbounds for j in bag
                CUDA.@atomic dx[row, j] += Δ[row, col] / bag_len
            end
        end
    end
    return nothing
...
function kernel_segmented_mean_back_w_aligned!(dx, dw, Δ, x, y, w, bags)
    idx = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    n_rows, n_cols = size(Δ)
    ...
    if idx <= n_rows * n_cols
        ...
        if !isempty(bag)
            ...
            @inbounds for j in bag
                CUDA.@atomic dx[row, j] += w[j] * Δ[row, col] / wsum
                CUDA.@atomic dw[j] += Δ[row, col] * (x[row, j] - y[row, col]) / wsum
            end
        end
    end
    return nothing
...
function segmented_mean_forw(x::CuMatrix, ψ::CuVector, bags::CuAlignedBags, w::Nothing)
    o = CUDA.zeros(eltype(x), size(x, 1), length(bags.bags))
    n_elements = length(o)
    if n_elements > 0
        threads, blocks = compute_grid_config(n_elements)
        @cuda threads=threads blocks=blocks kernel_segmented_mean_forw_aligned!(o, x, ψ, bags.bags)
    end
    o
end

function segmented_mean_forw(x::CuMatrix, ψ::CuVector, bags::CuAlignedBags, w::CuVector)
    o = CUDA.zeros(eltype(x), size(x, 1), length(bags.bags))
    n_elements = length(o)
    if n_elements > 0
        threads, blocks = compute_grid_config(n_elements)
        @cuda threads=threads blocks=blocks kernel_segmented_mean_forw_w_aligned!(o, x, ψ, w, bags.bags)
    end
    o
end

segmented_mean_forw(::Missing, ψ::CuVector, bags::CuAlignedBags, w) = repeat(ψ, 1, length(bags.bags))

# CuAlignedBags backward dispatches
function segmented_mean_back(Δ::CuMatrix, y, x::CuMatrix, ψ::CuVector, bags::CuAlignedBags, w::Nothing)
    dx = CUDA.zeros(eltype(x), size(x))
    dψ = CUDA.zeros(eltype(ψ), length(ψ))
    ...
    if n_elements > 0
        threads, blocks = compute_grid_config(n_elements)
        @cuda threads=threads blocks=blocks kernel_segmented_mean_back_aligned!(dx, Δ, bags.bags)
        @cuda threads=threads blocks=blocks kernel_sum_to_ψ!(dψ, Δ, bags.bags)
    end
...
function segmented_mean_back(Δ::CuMatrix, y::CuMatrix, x::CuMatrix, ψ::CuVector, bags::CuAlignedBags, w::CuVector)
    dx = CUDA.zeros(eltype(x), size(x))
    dw = CUDA.zeros(eltype(w), length(w))
    dψ = CUDA.zeros(eltype(ψ), length(ψ))
    ...
    if n_elements > 0
        threads, blocks = compute_grid_config(n_elements)
        @cuda threads=threads blocks=blocks kernel_segmented_mean_back_w_aligned!(dx, dw, Δ, x, y, w, bags.bags)
        @cuda threads=threads blocks=blocks kernel_sum_to_ψ!(dψ, Δ, bags.bags)
    end
...
function segmented_mean_back(Δ::CuMatrix, y, x::Missing, ψ::CuVector, bags::CuAlignedBags, w)
    dψ = CUDA.zeros(eltype(ψ), length(ψ))
    n_elements = length(Δ)
    if n_elements > 0
        threads, blocks = compute_grid_config(n_elements)
...
function ChainRulesCore.rrule(::typeof(segmented_mean_forw), x::CuMatrix, ψ::CuVector, bags::CuAlignedBags, w)
    y = segmented_mean_forw(x, ψ, bags, w)
    function segmented_mean_pullback(Δ)
        Δ_unthunked = ChainRulesCore.unthunk(Δ)
        grads = segmented_mean_back(Δ_unthunked, y, x, ψ, bags, w)
...
function ChainRulesCore.rrule(::typeof(segmented_mean_forw), x::Missing, ψ::CuVector, bags::CuAlignedBags, w)
    y = segmented_mean_forw(x, ψ, bags, w)
    function segmented_mean_pullback(Δ)
        Δ_unthunked = ChainRulesCore.unthunk(Δ)
        grads = segmented_mean_back(Δ_unthunked, y, x, ψ, bags, w)
