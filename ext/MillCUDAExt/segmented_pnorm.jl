...
function kernel_pnorm_precomp_aligned!(M, x, bags)
    idx = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    n_rows, n_cols = size(M)
    ...
    if idx <= n_rows * n_cols
        ...
        if isempty(bag)
            ...
            max_val = one(eltype(M))
            @inbounds for j in bag
                max_val = max(max_val, abs(x[row, j]))
            end
            @inbounds M[row, col] = max_val
        end
    end
...
function kernel_segmented_pnorm_forw_aligned!(o, a, ψ, p, M, bags)
    idx = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    n_rows, n_cols = size(o)
...
function kernel_segmented_pnorm_back_aligned!(da, dp, dψ, Δ, y, a, ψ, p, M, bags)
    idx = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    n_rows, n_cols = size(Δ)
    ...
    if idx <= n_rows * n_cols
        ...
        if isempty(bag)
            ...
            @inbounds for j in bag
                ab = abs(a[row, j])
                da_grad = δ * sign(a[row, j]) / count * (ab / yi) ^ (pi - one(eltype(a)))
                CUDA.@atomic da[row, j] += da_grad
            end
...
function pnorm_precomp_gpu(a::CuMatrix, bags::CuAlignedBags)
    M = CUDA.ones(eltype(a), size(a, 1), length(bags.bags))
    n_elements = length(M)
    if n_elements > 0
        threads, blocks = compute_grid_config(n_elements)
        @cuda threads=threads blocks=blocks kernel_pnorm_precomp_aligned!(M, a, bags.bags)
    end
    M
end

function segmented_pnorm_forw(a::CuMatrix, ψ::CuVector, p::CuVector, bags::CuAlignedBags, w::Nothing)
    M = pnorm_precomp_gpu(a, bags)
    o = CUDA.zeros(eltype(a), size(a, 1), length(bags.bags))
    n_elements = length(o)
    if n_elements > 0
        threads, blocks = compute_grid_config(n_elements)
        @cuda threads=threads blocks=blocks kernel_segmented_pnorm_forw_aligned!(o, a, ψ, p, M, bags.bags)
    end
    o, M
end

function Mill.segmented_pnorm_forw(a::CuMatrix, ψ::CuVector, p::CuVector, bags::CuAlignedBags, w)
    segmented_pnorm_forw(a, ψ, p, bags, w)[1]
end

segmented_pnorm_forw(::Missing, ψ::CuVector, p, bags::CuAlignedBags, w) = repeat(ψ, 1, length(bags.bags))

function segmented_pnorm_back(Δ::CuMatrix, y::CuMatrix, a::CuMatrix, ψ::CuVector, p::CuVector,
                              bags::CuAlignedBags, w::Nothing, M::CuMatrix)
    da = CUDA.zeros(eltype(a), size(a))
    dp = CUDA.zeros(eltype(p), length(p))
    dψ = CUDA.zeros(eltype(ψ), length(ψ))
    ...
    if n_elements > 0
        threads, blocks = compute_grid_config(n_elements)
        @cuda threads=threads blocks=blocks kernel_segmented_pnorm_back_aligned!(da, dp, dψ, Δ, y, a, ψ, p, M, bags.bags)
    end

    da, dψ, dp, ChainRulesCore.NoTangent(), ChainRulesCore.@not_implemented("Weight gradients not implemented for GPU PNorm")
end

function segmented_pnorm_back(Δ::CuMatrix, y, a::Missing, ψ::CuVector, bags::CuAlignedBags)
    dψ = CUDA.zeros(eltype(ψ), length(ψ))
    n_elements = length(Δ)
    if n_elements > 0
        threads, blocks = compute_grid_config(n_elements)
...
function ChainRulesCore.rrule(::typeof(Mill.segmented_pnorm_forw), a::CuMatrix, ψ::CuVector, p::CuVector, bags::CuAlignedBags, w)
    y, M = segmented_pnorm_forw(a, ψ, p, bags, w)
    function segmented_pnorm_pullback(Δ)
        Δ_unthunked = ChainRulesCore.unthunk(Δ)
        grads = segmented_pnorm_back(Δ_unthunked, y, a, ψ, p, bags, w, M)
...
function ChainRulesCore.rrule(::typeof(Mill.segmented_pnorm_forw), a::Missing, ψ::CuVector, p, bags::CuAlignedBags, w)
    y = segmented_pnorm_forw(a, ψ, p, bags, w)
    function segmented_pnorm_pullback(Δ)
        Δ_unthunked = ChainRulesCore.unthunk(Δ)
        grads = segmented_pnorm_back(Δ_unthunked, y, a, ψ, bags)
